...
class PlayerScoreSimulation(Simulation):
    def run_trial(self, selection_df, playing_chance=0.95, vice_captain=584):
        trial_df = selection_df.copy().merge(
            last_year_df[["code", "id"]], on="code", suffixes=("", "_last")
        )
        trial_df.index = selection_df.index
        trial_df["is_playing"] = playing_chance > np.random.uniform(
            0, 1, selection_df.shape[0]
        )
        trial_df["rand"] = np.random.uniform(0, 1, trial_df.shape[0])
        trial_df["score"] = (
            pd.merge(
                trial_df, score_distributions, left_on="id_last", right_on="element"
            )
            .groupby("element")
            .apply(lambda grp: grp[grp["p"] > grp["rand"]].iloc[0])["sampled_points"]
            .values
        )

        trial_df["score_if_playing"] = (
            trial_df["score"] * trial_df["is_playing"]
        ).fillna(0)
        ...
        first_team_missing = trial_df[trial_df["first_team"] & ~trial_df["is_playing"]]
        missing_by_position = (
            first_team_missing["element_type"].value_counts().to_dict()
        )
