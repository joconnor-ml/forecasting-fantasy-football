...
class TransferOptimiser:
    ...
    def instantiate_decision_arrays(self):
        ...
        transfer_in_decisions_free = get_decision_array(
            "transfer_in_free", self.num_players
        )
        transfer_in_decisions_paid = get_decision_array(
            "transfer_in_paid", self.num_players
        )
        transfer_out_decisions = get_decision_array(
            "transfer_out_paid", self.num_players
        )
    ...
    def apply_transfer_constraints(
        self,
        model,
        transfer_in_decisions_free,
        transfer_in_decisions,
    ...
    def solve(self, current_squad_indices, budget_now, sub_factor):
        ...
        (
            ...
            sub_decisions,
            captain_decisions,
        ) = self.instantiate_decision_arrays()

        # calculate new team from current team + transfers
        next_week_squad = (
            current_squad_decisions + transfer_in_decisions - transfer_out_decisions
        )
        ...
        self.apply_transfer_constraints(
            model,
            transfer_in_decisions_free,
            transfer_in_decisions,
            transfer_out_decisions,
        ...
        self.apply_formation_constraints(
            model,
            squad=next_week_squad,
            starters=starters,
            subs=sub_decisions,
            captains=captain_decisions,
        )
    ...
    def get_objective(
        self, starters, subs, captains, sub_factor, transfer_penalty, scores
    ):
    ...
    def apply_formation_constraints(self, model, squad, starters, subs, captains):
        for position, data in position_data.items():
            ...
            model += (
                sum(
                    starter
                    for starter, position in zip(starters, self.positions)
                    if position == data["position_id"]
                )
                >= data["min_starters"]
            )
            model += (
            ...
            model += (
                sum(
                    selected
                    for selected, position in zip(squad, self.positions)
                    if position == data["position_id"]
                )
                == data["num_total"]
            )
        ...
        for club_id in np.unique(self.clubs):
            model += (
                sum(
                    selected
                    for selected, club in zip(squad, self.clubs)
                    if club == club_id
                )
                <= 3
            )  # max 3 players
...
def get_decision_array_2d(name, n_players, n_weeks):
    return np.array(
        [
            [
                pulp.LpVariable(
                    "{}_{}_w{}".format(name, i, j), lowBound=0, upBound=1, cat="Integer"
                )
...
class MultiHorizonTransferOptimiser(TransferOptimiser):
    ...
    def __init__(
        self, expected_scores, buy_prices, sell_prices, positions, clubs, n_weeks
    ):
    ...
    def instantiate_decision_arrays(self):
        # we will make transfers in and out of the squad, and then pick subs and captains from that squad
        transfer_in_decisions_free = get_decision_array_2d(
            "transfer_in_free", self.num_players, self.num_weeks
        )
        transfer_in_decisions_paid = get_decision_array_2d(
            "transfer_in_paid", self.num_players, self.num_weeks
        )
        transfer_out_decisions = get_decision_array_2d(
            "transfer_out_paid", self.num_players, self.num_weeks
        )
        # total transfers in will be useful later
        transfer_in_decisions = [
            a + b
            for a, b in zip(transfer_in_decisions_free, transfer_in_decisions_paid)
        ]

        sub_decisions = get_decision_array_2d("subs", self.num_players, self.num_weeks)
        captain_decisions = get_decision_array_2d(
            "captain", self.num_players, self.num_weeks
        )
        return (
            transfer_in_decisions_free,
            transfer_in_decisions_paid,
    ...
    def solve(self, current_squad_indices, budget_now, sub_factor):
        ...
        model = pulp.LpProblem("Transfer optimisation", pulp.LpMaximize)
        (
            transfer_in_decisions_free_all,
            transfer_in_decisions_paid_all,
            transfer_out_decisions_all,
            transfer_in_decisions_all,
            sub_decisions_all,
            captain_decisions_all,
        ) = self.instantiate_decision_arrays()
        ...
        for w in range(self.num_weeks):
            ...
            # objective function:
            total_points += self.get_objective(
                starters,
                sub_decisions,
                captain_decisions,
                sub_factor,
                transfer_penalty,
                self.expected_scores[w],
            )
...
def get_transfer_dfs(transfer_in_decisions, transfer_out_decisions, player_df, week=0):
    transfers_in = [
        i
    ...
    transfers_out = [
        i
        for i in range(len(transfer_out_decisions[week]))
        if transfer_in_decisions[week][i].value() == 1
    ]
    return player_df.loc[transfers_in], player_df.loc[transfers_out]
